  ...
  async create(reqBody) {
    const record = await Person.create(reqBody).catch((error) => {
      if (Object.keys(error.keyPattern)[0] === 'email')
        throw new BadRequest(`email ${reqBody.email} is already in use`);
      if (Object.keys(error.keyPattern)[0] === 'cpf') throw new BadRequest(`cpf ${reqBody.cpf} is already in use`);
    });
    return record;
  }
